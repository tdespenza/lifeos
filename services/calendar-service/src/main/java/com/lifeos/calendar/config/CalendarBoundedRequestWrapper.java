...
/** Streams a request body through a hard byte limit, including chunked requests. */
final class CalendarBoundedRequestWrapper extends HttpServletRequestWrapper {

    private final long maximumBytes;
    private ServletInputStream inputStream;
    private BufferedReader reader;

    CalendarBoundedRequestWrapper(HttpServletRequest request, long maximumBytes) {
        super(request);
        if (maximumBytes < 1) {
    ...
    private static final class BoundedServletInputStream extends ServletInputStream {
        ...
        public int read(byte[] buffer, int offset, int length) throws IOException {
            ...
            if (remainingPlusOne <= 0) {
                int value = delegate.read();
                if (value >= 0) {
                    throw new CalendarPayloadTooLargeException();
                }
                return value;
            }
        ...
        private void count(int count) throws CalendarPayloadTooLargeException {
            bytesRead += count;
