-- Document Vault stores only metadata and opaque object references. There is no BLOB/BYTEA/content
-- column: bytes are admitted and verified exclusively by a DocumentObjectStore implementation.
CREATE TABLE vault_document (
    ...
    CONSTRAINT ck_vault_document_checksum CHECK (checksum_sha256 ~ '^[0-9a-f]{64}$'),
...
-- A command preserves its initial response snapshot. Raw client keys and raw request bodies are
-- not durable data; both digests are fixed-size SHA-256/HMAC values.
...
-- Redacted audit facts are separate from document metadata and do not contain target identifiers.
