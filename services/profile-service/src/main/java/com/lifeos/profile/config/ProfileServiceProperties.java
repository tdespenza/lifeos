...
/** Deployment-owned resource bounds and digest secrets for the Profile service. */
...
public class ProfileServiceProperties {
    ...
    @NotBlank(message = "idempotencySecret (PROFILE_IDEMPOTENCY_SECRET) must be configured and non-blank")
    private String idempotencySecret;

    @NotBlank(message = "auditClientFingerprintSecret (PROFILE_AUDIT_CLIENT_FINGERPRINT_SECRET) must be configured and non-blank")
