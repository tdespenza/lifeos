...
import com.lifeos.calendar.config.CalendarIdentityProperties;
...
/** Fail-closed, bounded adapter for Identity validation and authorization decisions. */
@Component
public class RestClientCalendarAccessService implements CalendarAccessService {
    ...
    private final CalendarIdentityProperties properties;
    private final Semaphore outboundPermits;

    @Autowired
    public RestClientCalendarAccessService(
            RestClient.Builder restClientBuilder, CalendarIdentityProperties properties) {
        this(buildRestClient(restClientBuilder, properties), properties);
    }

    RestClientCalendarAccessService(RestClient restClient, CalendarIdentityProperties properties) {
    ...
    @Override
    public CalendarSubject authenticate(String authorizationHeader) {
        ...
        } catch (CalendarAuthenticationFailure | CalendarAuthorizationDependencyUnavailable exception) {
            throw exception;
        } catch (RestClientResponseException exception) {
            if (exception.getStatusCode().value() == 401) {
                throw new CalendarAuthenticationFailure(exception);
            }
            throw new CalendarAuthorizationDependencyUnavailable(exception);
    ...
    @Override
    public void authorize(CalendarSubject subject, String action, CalendarAuthorizationResource resource) {
        try {
            withOutboundPermit(() -> {
                ...
                if (!isUsableDecision(response) || "POLICY_UNAVAILABLE".equals(response.reasonCode())) {
                    throw new CalendarAuthorizationDependencyUnavailable();
                }
                if (!"ALLOW".equals(response.outcome())) {
                    throw new CalendarAuthorizationDenied();
                }
                return null;
            });
        } catch (CalendarAuthorizationDenied | CalendarAuthorizationDependencyUnavailable exception) {
            throw exception;
        } catch (RuntimeException exception) {
            throw new CalendarAuthorizationDependencyUnavailable(exception);
        }
    ...
    private static RestClient buildRestClient(RestClient.Builder builder, CalendarIdentityProperties properties) {
    ...
    private static String extractBearerToken(String header) {
        if (header == null || !header.regionMatches(true, 0, "Bearer ", 0, "Bearer ".length())) {
            throw new CalendarAuthenticationFailure();
        }
    ...
    private static CalendarSubject toSubject(ValidatedSubjectResponse response) {
        if (response == null
