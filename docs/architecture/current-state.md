...
- **`services/identity-service`** (port 8081) — account registration over its own PostgreSQL database (`lifeos_identity`). No authentication yet; see [`docs/api/identity-service.md`](../api/identity-service.md).
...
- **Redis is provisioned but unused** — it runs in docker-compose so the infra exists, but no service has caching, session storage, or rate limiting wired to it yet.
- **No auth** — identity-service models account registration only, not credentials, OAuth2/OIDC, JWT, or passkeys. This was a deliberate scope decision (see [`docs/api/identity-service.md`](../api/identity-service.md)) rather than an oversight: half-implementing password handling to check a box would be worse than not implementing it yet.
- **No central observability stack yet** — identity-service now emits ECS-structured stdout logs, Micrometer/Prometheus metrics, and OpenTelemetry traces, and exposes liveness/readiness probes. Prometheus/Grafana/Loki/Tempo collection and dashboards are not deployed yet, and task-goal-service has not received the same instrumentation.
