...
- Virtual threads are not a free performance win: as of JEP 491 (Java 24, carried into 25), ordinary `synchronized` blocks no longer pin the carrier thread, but native calls (JNI, Foreign Function & Memory API) still do, and thread-local-heavy libraries (older JDBC drivers, some connection pools) can still degrade throughput under load through other mechanisms. Every blocking dependency (HikariCP, JDBC drivers, Kafka client) must be verified as virtual-thread-safe, not assumed.
