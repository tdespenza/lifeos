...
- **`services/identity-service`** (port 8081) — account registration, first-party email/password login, configured OAuth2/OIDC authorization-code login, passkey/WebAuthn assertion login, short-lived JWT/JWKS validation, and one-time refresh-token rotation over its own PostgreSQL database (`lifeos_identity`). Passkey credential registration/provisioning, RBAC/ABAC, and user-facing session management remain planned; see [`docs/api/identity-service.md`](../api/identity-service.md).
...
- **Authentication is partial** — identity-service now stores Argon2id password credentials, WebAuthn public-key metadata/counters, verified provider-subject mappings, durable token families and replay evidence; issues bounded access JWTs and opaque refresh tokens through the shared session authority; publishes configured public JWKS; validates JWTs plus durable session state; and returns generic failures. Passkey credential registration, key rotation windows, RBAC/ABAC, gateway enforcement, and user-facing session listing/revocation remain planned stories.
