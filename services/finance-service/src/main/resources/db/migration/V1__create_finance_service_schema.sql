-- Finance owns an isolated system of record. Identity account UUIDs are opaque references that
-- are authenticated and authorized at the workload boundary; no cross-service foreign key exists.
CREATE TABLE finance_budget (
    ...
    CONSTRAINT ck_finance_budget_currency CHECK (currency ~ '^[A-Z]{3}$')
);

CREATE INDEX idx_finance_budget_owner_tenant_period
    ON finance_budget (owner_account_id, tenant_id, period_start, period_end);

-- PostgreSQL's exclusion constraint is the final concurrency invariant: distinct idempotency keys
-- cannot commit two overlapping budgets for one owner, tenant, and category. btree_gist supplies
-- equality operators for UUID/text alongside the date-range overlap operator.
CREATE EXTENSION IF NOT EXISTS btree_gist;
ALTER TABLE finance_budget ADD CONSTRAINT ex_finance_budget_owner_tenant_category_period
    EXCLUDE USING gist (
...
CREATE TABLE financial_transaction (
    ...
    CONSTRAINT ck_financial_transaction_currency CHECK (currency ~ '^[A-Z]{3}$')
);
...
-- Corrections form the auditable category history. Transaction monetary/posting columns are
-- immutable after insertion; no update path can rewrite amount, direction, date, or currency.
...
CREATE TABLE financial_goal (
    ...
    CONSTRAINT ck_financial_goal_currency CHECK (currency ~ '^[A-Z]{3}$')
);
...
-- Raw idempotency keys and request content are never stored. The row atomically stores the exact
-- immutable response snapshot/status/location with the posting or conditional mutation.
...
CREATE INDEX idx_finance_mutation_idempotency_resource
    ON finance_mutation_idempotency (resource_id);

-- Security audit facts deliberately exclude financial names, categories, amounts, merchant data,
-- bearer tokens, raw idempotency keys, request payloads, and client network addresses.
