-- Durable, owner-scoped request only. A separate worker must consume the outbox and obtain any
-- external Trust Ledger/Besu proof; this migration intentionally makes no external claim.
CREATE TABLE document_proof_request (
    ...
    CONSTRAINT ck_document_proof_request_checksum CHECK (checksum_sha256 ~ '^[0-9a-f]{64}$'),
...
CREATE TABLE document_proof_outbox_event (
    ...
    CONSTRAINT ck_document_proof_outbox_checksum CHECK (checksum_sha256 ~ '^[0-9a-f]{64}$')
);
