...
See [README.md](README.md) for the project overview and [REQUIREMENTS.md](REQUIREMENTS.md) (gitignored — generated locally, not tracked in git) for the full product vision, architecture, and roadmap.

## Documentation conventions

* **Architecture Decision Records** live in `docs/adr/`. Per [CLAUDE.md](CLAUDE.md) / [AGENT.md](AGENT.md), create or update an ADR whenever a decision affects architecture, data models, scaling strategy, security model, deployment model, external dependencies, API contracts, messaging, persistence, AI providers, or blockchain assumptions.
...
Every meaningful change should include tests or a documented reason why tests don't apply — see [CLAUDE.md](CLAUDE.md)'s Testing Strategy section. Run `./gradlew build` before opening a PR; it compiles and runs the full test suite.
