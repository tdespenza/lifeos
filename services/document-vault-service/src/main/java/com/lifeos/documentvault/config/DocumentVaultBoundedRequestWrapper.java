...
/** Enforces body admission limits for requests that omit Content-Length. */
final class DocumentVaultBoundedRequestWrapper extends HttpServletRequestWrapper {

    private final long maximumBytes;
    private ServletInputStream inputStream;
    private BufferedReader reader;

    DocumentVaultBoundedRequestWrapper(HttpServletRequest request, long maximumBytes) {
        super(request);
    ...
    private static final class BoundedServletInputStream extends ServletInputStream {
        ...
        public int read(byte[] buffer, int offset, int length) throws IOException {
            ...
            if (remainingPlusOne <= 0) {
                int value = delegate.read();
                if (value >= 0) {
                    throw new DocumentVaultPayloadTooLargeException();
                }
                return value;
            }
            int read = delegate.read(buffer, offset, (int) Math.min(length, remainingPlusOne));
            if (read > 0) {
                count(read);
            }
            return read;
