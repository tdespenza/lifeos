...
import com.lifeos.finance.config.FinanceIdentityProperties;
...
/**
 * Fail-closed, bounded REST adapter for Identity authentication and V2 decisions. It retains no
 * bearer or workload credential after a call and redacts both from exception and record output.
 */
@Component
public class RestClientFinanceAccessService implements FinanceAccessService {
    ...
    private final FinanceIdentityProperties properties;
    private final Semaphore outboundPermits;

    @Autowired
    public RestClientFinanceAccessService(RestClient.Builder restClientBuilder, FinanceIdentityProperties properties) {
        this(buildRestClient(restClientBuilder, properties), properties);
    }

    RestClientFinanceAccessService(RestClient restClient, FinanceIdentityProperties properties) {
    ...
    @Override
    public FinanceSubject authenticate(String authorizationHeader) {
        ...
        } catch (FinanceAuthenticationFailure | FinanceAuthorizationDependencyUnavailable exception) {
            throw exception;
        } catch (RestClientResponseException exception) {
            if (exception.getStatusCode().value() == 401) {
                throw new FinanceAuthenticationFailure(exception);
            }
            throw new FinanceAuthorizationDependencyUnavailable(exception);
    ...
    @Override
    public void authorize(FinanceSubject subject, String action, FinanceAuthorizationResource resource) {
        try {
            withOutboundPermit(() -> {
                ...
                if (!isUsableDecision(response) || "POLICY_UNAVAILABLE".equals(response.reasonCode())) {
                    throw new FinanceAuthorizationDependencyUnavailable();
                }
                if (!"ALLOW".equals(response.outcome())) {
                    throw new FinanceAuthorizationDenied();
                }
                return null;
            });
        } catch (FinanceAuthorizationDenied | FinanceAuthorizationDependencyUnavailable exception) {
            throw exception;
        } catch (RuntimeException exception) {
            throw new FinanceAuthorizationDependencyUnavailable(exception);
        }
    ...
    private static RestClient buildRestClient(RestClient.Builder builder, FinanceIdentityProperties properties) {
    ...
    private static String extractBearerToken(String header) {
        if (header == null || !header.regionMatches(true, 0, "Bearer ", 0, "Bearer ".length())) {
            throw new FinanceAuthenticationFailure();
        }
    ...
    private static FinanceSubject toSubject(ValidatedSubjectResponse response) {
        if (response == null
