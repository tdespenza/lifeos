...
/**
 * Validates the database contract before identity-service creates a connection pool.
 *
 * <p>The application's deployment configuration maps these values from the named identity
...
@Validated
public class IdentityDatasourceProperties {

    @NotBlank(message = "spring.datasource.url (IDENTITY_DATASOURCE_URL) must be configured and non-blank")
    private String url;

    @NotBlank(message = "spring.datasource.username (IDENTITY_DATASOURCE_USERNAME) must be configured and non-blank")
    private String username;

    @NotBlank(message = "spring.datasource.password (IDENTITY_DATASOURCE_PASSWORD) must be configured and non-blank")
