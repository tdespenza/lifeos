...
/** Applies one bounded deadline to request connection, keep-alive, and multipart upload reads. */
@Configuration
public class DocumentVaultInboundRequestConfiguration {

    @Bean
    public WebServerFactoryCustomizer<TomcatServletWebServerFactory> documentVaultInboundTimeoutCustomizer(
    ...
    static void configureConnector(ProtocolHandler handler, Duration timeout) {
        if (!(handler instanceof AbstractHttp11Protocol<?> protocol)) {
            throw new IllegalStateException(
                    "document-vault-service requires a Tomcat HTTP/1.1 connector for upload timeout enforcement");
        }
