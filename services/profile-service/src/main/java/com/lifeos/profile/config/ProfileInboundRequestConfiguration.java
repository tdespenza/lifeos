...
/** Applies the bounded request-line, keep-alive, and request-body timeout to Tomcat. */
@Configuration
public class ProfileInboundRequestConfiguration {

    @Bean
    public WebServerFactoryCustomizer<TomcatServletWebServerFactory> profileInboundRequestTimeoutCustomizer(
            ProfileServiceProperties properties) {
    ...
    static void configureConnector(ProtocolHandler handler, Duration timeout) {
        if (!(handler instanceof AbstractHttp11Protocol<?> protocol)) {
            throw new IllegalStateException(
                    "profile-service requires a Tomcat HTTP/1.1 connector to enforce inbound request-body timeouts");
        }
