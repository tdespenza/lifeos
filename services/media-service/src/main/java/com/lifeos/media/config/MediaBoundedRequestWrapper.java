...
/** Streams chunked request bodies through a byte ceiling rather than trusting Content-Length. */
final class MediaBoundedRequestWrapper extends HttpServletRequestWrapper {

    private final long maximumBytes;
    private ServletInputStream inputStream;
    private BufferedReader reader;

    MediaBoundedRequestWrapper(HttpServletRequest request, long maximumBytes) {
        super(request);
        if (maximumBytes < 1) {
    ...
    private static final class BoundedServletInputStream extends ServletInputStream {
        ...
        public int read(byte[] buffer, int offset, int length) throws IOException {
            ...
            if (remainingPlusOne <= 0) {
                int value = delegate.read();
                if (value >= 0) {
                    throw new MediaPayloadTooLargeException();
                }
                return value;
            }
            int permitted = (int) Math.min(length, remainingPlusOne);
            int read = delegate.read(buffer, offset, permitted);
            if (read > 0) {
                count(read);
            }
            return read;
