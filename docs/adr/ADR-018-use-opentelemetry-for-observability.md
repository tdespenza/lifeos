...
- **Micrometer + Prometheus only (metrics, no distributed tracing):** Simpler to stand up, and Micrometer already integrates natively with Spring Boot Actuator. Rejected because it drops distributed tracing entirely — across a twelve-module call graph, trace-level causality (which downstream call caused the p99 spike) is exactly the signal most valuable to demonstrate and most useful for real debugging; metrics alone answer "something is slow" but not "where."
- **ELK (Elasticsearch, Logstash, Kibana) for logs:** Mature, widely deployed in industry, powerful full-text search. Rejected in favor of Loki/Grafana because ELK's resource footprint (JVM-heavy Elasticsearch cluster, Logstash pipelines) is disproportionate for a personal-scale lab environment; Loki's label-indexed, log-line-compressed model is far cheaper to run alongside the other service modules on modest hardware while still integrating into the same Grafana surface as metrics and traces.
