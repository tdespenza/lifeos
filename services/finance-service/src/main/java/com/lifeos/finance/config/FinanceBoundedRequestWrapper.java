...
final class FinanceBoundedRequestWrapper extends HttpServletRequestWrapper {

    private final long maximumBytes;
    private ServletInputStream inputStream;
    private BufferedReader reader;

    FinanceBoundedRequestWrapper(HttpServletRequest request, long maximumBytes) {
        super(request);
        if (maximumBytes < 1) {
    ...
    private static final class BoundedServletInputStream extends ServletInputStream {
        ...
        public int read(byte[] buffer, int offset, int length) throws IOException {
            ...
            if (remainingPlusOne <= 0) {
                int value = delegate.read();
                if (value >= 0) {
                    throw new FinancePayloadTooLargeException();
                }
                return value;
            }
        ...
        private void count(int count) throws FinancePayloadTooLargeException {
            bytesRead += count;
