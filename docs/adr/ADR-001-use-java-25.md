...
- **Java 21 LTS (stay on prior LTS)** — safer, more battle-tested ecosystem. Rejected as primary because Structured Concurrency (JEP 505) and Scoped Values (JEP 506) are only finalized in Java 25; on 21 they remain preview APIs that changed shape across 21–24. Building the orchestrator and algorithm engine's concurrency model on a preview API means coding against a moving target.
