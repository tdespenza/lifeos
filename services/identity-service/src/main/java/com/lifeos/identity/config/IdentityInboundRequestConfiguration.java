...
/**
 * Applies the identity service's bounded inbound timeout policy to Tomcat's HTTP connector.
 *
 ...
 */
@Configuration
public class IdentityInboundRequestConfiguration {

    /**
     * Installs the inbound timeout policy for every identity-service Tomcat connector.
     *
     * @param properties identity service resource-bound configuration
     * @return Tomcat factory customizer
     */
    @Bean
    public WebServerFactoryCustomizer<TomcatServletWebServerFactory> identityInboundRequestTimeoutCustomizer(
     ...
     * @param timeout bounded identity-service timeout
     */
    static void configureConnector(ProtocolHandler handler, Duration timeout) {
        if (!(handler instanceof AbstractHttp11Protocol<?> protocol)) {
            throw new IllegalStateException(
                    "identity-service requires a Tomcat HTTP/1.1 connector to enforce inbound request-body timeouts");
        }
