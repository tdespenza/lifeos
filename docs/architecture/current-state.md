...
- **`services/identity-service`** (port 8081) — account registration and first-party email/password login over its own PostgreSQL database (`lifeos_identity`). OAuth2/OIDC, passkeys/WebAuthn, refresh-token rotation, RBAC/ABAC, and user-facing session management remain planned; see [`docs/api/identity-service.md`](../api/identity-service.md).
...
- **Redis is used only by identity-service login rate limiting** — it runs in docker-compose and stores bounded SHA-256-keyed attempt counters. Durable sessions remain in PostgreSQL; Redis failures fail closed for login. No shared cache, refresh-token rotation, or gateway rate limiting is implemented yet.
- **Authentication is partial** — identity-service now stores Argon2id password credentials, issues short-lived HS256 access tokens through the shared session authority, persists session/token digests, and returns generic failures. OAuth2/OIDC, passkeys/WebAuthn, asymmetric key/JWKS rotation, refresh tokens, RBAC/ABAC, gateway enforcement, and user-facing session listing/revocation remain planned stories.
