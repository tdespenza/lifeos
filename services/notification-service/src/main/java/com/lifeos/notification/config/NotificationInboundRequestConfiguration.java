...
@Configuration
public class NotificationInboundRequestConfiguration {

    @Bean
    public WebServerFactoryCustomizer<TomcatServletWebServerFactory> notificationInboundRequestTimeoutCustomizer(
    ...
    static void configureConnector(ProtocolHandler handler, Duration timeout) {
        if (!(handler instanceof AbstractHttp11Protocol<?> protocol)) {
            throw new IllegalStateException(
                    "notification-service requires a Tomcat HTTP/1.1 connector for bounded request reads");
        }
