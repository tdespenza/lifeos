...
@Configuration
public class AssistantInboundRequestConfiguration {

    @Bean
    public WebServerFactoryCustomizer<TomcatServletWebServerFactory> assistantInboundRequestTimeoutCustomizer(
    ...
    static void configureConnector(ProtocolHandler handler, Duration timeout) {
        if (!(handler instanceof AbstractHttp11Protocol<?> protocol)) {
            throw new IllegalStateException(
                    "ai-assistant-service requires a Tomcat HTTP/1.1 connector to enforce inbound request-body timeouts");
        }
