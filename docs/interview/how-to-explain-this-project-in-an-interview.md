...
Right now, two services are running. identity-service handles account registration — nothing more, no auth yet, deliberately deferred. task-goal-service does goal CRUD, plus the piece I'm actually proud of: a dependency-ordering endpoint that runs a real topological sort — Kahn's algorithm — over a goal dependency graph, and returns an HTTP 409 with cycle detection instead of silently returning a broken ordering when the graph is invalid. That's a genuine graph algorithm solving a real correctness problem, not a toy.
