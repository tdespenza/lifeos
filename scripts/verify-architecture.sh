...
for service_directory in "${SERVICES[@]}"; do
    ...
    if ! rg --quiet 'id\("org\.springframework\.boot"\)' "${service_directory}/build.gradle.kts"; then
        echo "${service} must explicitly apply the Spring Boot plugin" >&2
        exit 65
    fi

    if ! rg --quiet '@SpringBootApplication' "${main_source_directory}"; then
        echo "${service} has no Spring Boot application entry point" >&2
        exit 65
    fi

    if ! rg --quiet '^USER [^[:space:]]+' "${dockerfile}" \
        || rg --quiet '^USER (root|0)(:|$)' "${dockerfile}"; then
        echo "${service} Dockerfile must run as a non-root user" >&2
        exit 65
    fi

    if ! rg --quiet '^ARG JAR_FILE$' "${dockerfile}" \
        || ! rg --quiet '^COPY .*\$\{JAR_FILE\} /app/app\.jar$' "${dockerfile}"; then
    ...
    if rg '^FROM ' "${dockerfile}" | rg --invert-match --quiet '@sha256:[0-9a-f]{64}([[:space:]]|$)'; then
    ...
    done < <(rg --no-filename --only-matching '^import com\.lifeos\.[A-Za-z0-9_.]+' "${main_source_directory}" || true)
