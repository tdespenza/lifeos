-- H2 test equivalent of the production profile-service Flyway schema. It deliberately remains a
-- separate test migration location so PostgreSQL-only syntax cannot silently be relied upon.
...
CREATE INDEX idx_personal_profile_owner_tenant ON personal_profile (owner_account_id, tenant_id);
...
CREATE INDEX idx_household_owner_tenant ON household (owner_account_id, tenant_id);
...
CREATE INDEX idx_household_member_account ON household_member (member_account_id, household_id);
...
CREATE INDEX idx_profile_mutation_idempotency_resource ON profile_mutation_idempotency (resource_id);
