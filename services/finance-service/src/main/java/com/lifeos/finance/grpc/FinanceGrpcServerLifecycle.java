...
/** Starts the optional internal finance gRPC host only with mTLS and workload credentials. */
public final class FinanceGrpcServerLifecycle implements SmartLifecycle {

    private final FinanceMetricsGrpcService metricsService;
    private final GrpcServerProperties properties;
    private Server server;

    public FinanceGrpcServerLifecycle(FinanceMetricsGrpcService metricsService, GrpcServerProperties properties) {
    ...
    @Override public void start() {
        ...
        try {
            ...
            throw new IllegalStateException("finance gRPC mTLS server failed to start", exception);
        }
    }
    ...
    private void validateProperties() {
        if (properties.getPort() < 1_024 || properties.getPort() > 65_535 || !properties.isTlsEnabled()
            ...
            throw new IllegalStateException(
                    "enabled finance gRPC requires a bounded port, mTLS certificate/key/trust files, and workload token");
        }
    }
}
