-- H2 equivalent of the production schema. Keep this explicit instead of using Hibernate DDL so
-- tests validate the same durable ownership and no-content-retention contract.
...
CREATE TABLE assistant_request_audit_event (
    ...
    CONSTRAINT ck_assistant_request_audit_input_fingerprint CHECK (
        input_fingerprint IS NULL OR REGEXP_LIKE(input_fingerprint, '^[0-9a-f]{64}$')
    ),
    CONSTRAINT ck_assistant_request_audit_client_fingerprint CHECK (
        REGEXP_LIKE(client_fingerprint, '^[0-9a-f]{64}$')
    ),
    CONSTRAINT ck_assistant_request_audit_output_fingerprint CHECK (
        output_fingerprint IS NULL OR REGEXP_LIKE(output_fingerprint, '^[0-9a-f]{64}$')
    ),
