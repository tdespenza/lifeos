...
- **`services/identity-service`** (port 8081) — account registration, first-party email/password login, and configured OAuth2/OIDC authorization-code login over its own PostgreSQL database (`lifeos_identity`). Passkeys/WebAuthn, refresh-token rotation, RBAC/ABAC, and user-facing session management remain planned; see [`docs/api/identity-service.md`](../api/identity-service.md).
...
- **Redis is used by identity-service login rate limiting and OIDC callback state** — it stores bounded HMAC-keyed attempt counters and short-lived, single-use authorization state. Durable sessions and external-identity mappings remain in PostgreSQL; Redis failures fail closed for authentication. No shared cache, refresh-token rotation, or gateway rate limiting is implemented yet.
- **Authentication is partial** — identity-service now stores Argon2id password credentials and verified provider-subject mappings, issues short-lived HS256 access tokens through the shared session authority, persists session/token digests, and returns generic failures. Passkeys/WebAuthn, asymmetric key/JWKS rotation, refresh tokens, RBAC/ABAC, gateway enforcement, and user-facing session listing/revocation remain planned stories.
